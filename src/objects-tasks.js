...
function makeWord(lettersObject) {
  ...
  const maxVal = Math.max(...Object.values(lettersObject).flat(), 0);
  const res = new Array(maxVal + 1).fill('-');
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) change += 25;
    else {
      const changeReq = queue[i] - 25;
      if (changeReq > change) return false;
      change = change - changeReq + 25;
    }
  }
  return true;
}
